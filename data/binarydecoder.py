...
class BinaryRecord:
    ...
    def __init__(self, start_bytes, binary_fields, field_types = [], endianness = '<'):
        ...
        if p != None and (self.field_types[p.type].size != 1 or p.count != 1):
        ...
        c = d.get(BinaryRecord._checksum_name)
        if c != None and c.count != 1:
            raise ValueError(("if there is a BinaryField with name = '%s', " +
                             "it must be have count = 1") %
                             BinaryRecord._checksum_name)
        self.checksum_size = self.field_types[c.type].size if c != None else 0
    ...
    """
    def status(self):
        return 'seeks: %d  parity/checksum fails: %d  valid: %d' % \
                (self.byte_seeks, self.validation_fails, self.validation_wins)
    ...
    def try_parse(self, by):
        ...
        if self.parity_size > 0:
            ...
            record_valid = record_valid and parity == rec.__dict__[BinaryRecord._parity_name]
        
        if self.checksum_size > 0:
            checksum = sum(by[:self.data_byte_count - self.checksum_size]) & (1 << self.checksum_size * 8) - 1
            record_valid = record_valid and checksum == rec.__dict__[BinaryRecord._checksum_name]
        
        return rec if record_valid else None
    ...
    def decode(self, by):
        ...
        records = []
        i = 0
        using_validation = self.parity_size + self.checksum_size > 0
